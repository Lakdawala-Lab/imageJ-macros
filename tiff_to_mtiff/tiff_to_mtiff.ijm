...
dayf="20170927\\"; //folder named according to date on which data was acquired
day_dir_path=rootd+dayf;
list_day = getFileList(day_dir_path);
run_beg=1;
run_end = 2;//list_day.length;
for (run_idx=run_beg; run_idx<run_end; run_idx++){ //list_day.length; run_idx++){
	...
	mt_dir_path=rootd+dayf+df+"\\ARM1_mtiff\\"; 
	...
	for(i=0;i<tot_n;i++){
		...
		if(File.exists(rootd+dayf+df+"\\ARM1\\"+df+"_A"+IJ.pad(i,3)+"\\"+df+"_AT"+IJ.pad(i,3)+"Z00.tif")){
			run("Image Sequence...", "open=["+rootd+dayf+df+"\\ARM1\\"+df+"_A"+IJ.pad(i,3)+"\\"+df+"_AT"+IJ.pad(i,3)+"Z00.tif] sort");
			//run("Properties...", "channels=1 slices="+tot_frm+" frames=1 unit=um pixel_width=0.1625 pixel_height=0.1625 voxel_depth=0.5");
			saveAs("Tiff", rootd+dayf+df+"\\ARM1_mtiff\\"+df+"_A"+IJ.pad(i,3)+".tif");  
			close();
		}
	}
	...
	mt_dir_path=rootd+dayf+df+"\\ARM2_mtiff\\"; 
	if (File.isDirectory(mt_dir_path) == 0)
		File.makeDirectory(mt_dir_path);
	for(i=0;i<tot_n;i++){
		//end_str=indexOf(dlist[i],"/");
		//IJ.redirectErrorMessages();
		if(File.exists(rootd+dayf+df+"\\ARM2\\"+df+"_B"+IJ.pad(i,3)+"\\"+df+"_BT"+IJ.pad(i,3)+"Z00.tif")){
			run("Image Sequence...", "open=["+rootd+dayf+df+"\\ARM2\\"+df+"_B"+IJ.pad(i,3)+"\\"+df+"_B"+IJ.pad(i,3)+"Z00.tif] sort");
			//run("Properties...", "channels=1 slices="+tot_frm+" frames=1 unit=um pixel_width=0.1625 pixel_height=0.1625 voxel_depth=0.5");
			saveAs("Tiff", rootd+dayf+df+"\\ARM2_mtiff\\"+df+"_B"+IJ.pad(i,3)+".tif"); 
			close();
		}
	}
}
